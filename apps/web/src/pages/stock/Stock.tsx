      ...
      {isLoading ? <SkeletonTable rows={10} cols={6} /> : (
        <>
          <Table
            ...
            data={items} keyExtractor={(s) => s.id} emptyMessage="لا توجد بيانات مخزون" emptyDescription="أضف منتجات لتتبّع مخزونك" emptyIcon={<Package className="w-8 h-8" />}
          />
      ...
      {isLoading ? <SkeletonTable rows={10} cols={6} /> : (
        <>
          <Table
            ...
            data={items} keyExtractor={(m) => m.id} emptyMessage="لا توجد حركات مخزون" emptyDescription="ستظهر هنا حركات المخزون تلقائياً" emptyIcon={<TrendingUp className="w-8 h-8" />}
          />
          {meta && <Pagination page={meta.page} pages={meta.pages} total={meta.total} limit={meta.limit} onPage={setPage} />}
        </>
      )}
      ...
      {isLoading ? <SkeletonTable rows={8} cols={6} /> : (
        <>
          <Table
            ...
            data={items} keyExtractor={(t) => t.id} emptyMessage="لا توجد طلبات تحويل" emptyDescription="حوّل المخزون بين الفروع" emptyIcon={<ArrowLeftRight className="w-8 h-8" />}
          />
          {meta && <Pagination page={meta.page} pages={meta.pages} total={meta.total} limit={meta.limit} onPage={setPage} />}
        </>
      )}
